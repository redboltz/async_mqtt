...
public:
    ...
    basic_endpoint() = delete;
    basic_endpoint(this_type&&) = default;
    basic_endpoint(this_type const&) = delete;
    basic_endpoint& operator=(this_type&&) = default;
    basic_endpoint& operator=(this_type const&) = delete;
...
private:
    ...
    struct impl : std::enable_shared_from_this<impl> {
        ...
        impl(this_type const&) = delete;
        impl(this_type&&) = delete;
...
public:
    ...
    basic_endpoint(
        protocol_version ver,
        Args&&... args
    ):impl_{std::make_shared<impl>(ver, std::forward<Args>(args)...)}
    {}
     ...
     */
    void set_pingresp_recv_timeout_ms(std::size_t ms) {
        impl_->set_pingresp_recv_timeout_ms(ms);
    }
    ...
    register_packet_id(
        ...
        return impl_->register_packet_id(
            packet_id,
            std::forward<CompletionToken>(token)
        );
    }
    ...
    release_packet_id(
        ...
        return impl_->release_packet_id(
            packet_id,
            std::forward<CompletionToken>(token)
        );
    }
    ...
    regulate_for_store(
        ...
        return impl_->regulate_for_store(
            force_move(packet),
            std::forward<CompletionToken>(token)
        );
    }
     ...
     */
    bool register_packet_id(packet_id_t pid) {
        return impl_->register_packet_id(pid);
    }
     ...
     */
    void regulate_for_store(v5::basic_publish_packet<PacketIdBytes>& packet) const {
        impl_->regulate_for_store(packet);
    }

    void cancel_all_timers_for_test() {
        impl_->cancel_all_timers_for_test();
    }

    void set_pingreq_send_interval_ms_for_test(std::size_t ms) {
        impl_->set_pingreq_send_interval_ms_for_test(ms);
    }
