...
#if !defined(ASYNC_MQTT_PACKET_IMPL_V5_PUBACK_IPP)
#define ASYNC_MQTT_PACKET_IMPL_V5_PUBACK_IPP
...
template <std::size_t PacketIdBytes>
ASYNC_MQTT_HEADER_ONLY_INLINE
std::vector<as::const_buffer> basic_puback_packet<PacketIdBytes>::const_buffer_sequence() const {
...
template <std::size_t PacketIdBytes>
ASYNC_MQTT_HEADER_ONLY_INLINE
std::size_t basic_puback_packet<PacketIdBytes>::size() const {
    return
        1 +                            // fixed header
...
template <std::size_t PacketIdBytes>
ASYNC_MQTT_HEADER_ONLY_INLINE
std::size_t basic_puback_packet<PacketIdBytes>::num_of_const_buffer_sequence() const {
    return
        1 +                   // fixed header
        1 +                   // remaining length
        1 +                   // packet_id
...
template <std::size_t PacketIdBytes>
ASYNC_MQTT_HEADER_ONLY_INLINE
puback_reason_code basic_puback_packet<PacketIdBytes>::code() const {
...
template <std::size_t PacketIdBytes>
ASYNC_MQTT_HEADER_ONLY_INLINE
properties const& basic_puback_packet<PacketIdBytes>::props() const {
    return props_;
}
...
basic_puback_packet<PacketIdBytes>::basic_puback_packet(
    ...
    : fixed_header_{
          detail::make_fixed_header(control_packet_type::puback, 0b0000)
      },
      remaining_length_{
          PacketIdBytes
      },
...
basic_puback_packet<PacketIdBytes>::basic_puback_packet(buffer buf, error_code& ec) {
    ...
    if (auto pl_opt = variable_bytes_to_val(it, buf.end())) {
        ...
        props_ = make_properties(prop_buf, property_location::puback, ec);
        if (ec) return;
...
template class basic_puback_packet<a_size>; \
template bool operator==(basic_puback_packet<a_size> const& lhs, basic_puback_packet<a_size> const& rhs); \
template bool operator<(basic_puback_packet<a_size> const& lhs, basic_puback_packet<a_size> const& rhs);
...
#endif // ASYNC_MQTT_PACKET_IMPL_V5_PUBACK_IPP
