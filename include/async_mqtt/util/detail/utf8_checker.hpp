...
#if !defined(ASYNC_MQTT_UTIL_DETAIL_UTF8_CHECKER_HPP)
#define ASYNC_MQTT_UTIL_DETAIL_UTF8_CHECKER_HPP
        ...
        // Align `in` to sizeof(std::size_t) boundary
        {
            ...
            while(in < last)
            {
                if(*in & 0x80)
                {
                    ...
                    size = size - static_cast<std::size_t>(in - in0);
                    goto slow;
                }
                ++in;
            }
...
tail:
        ...
        for(;;)
        {
            ...
            else
            {
                ...
                need_ = static_cast<std::size_t>(need - n);
...
#endif // ASYNC_MQTT_UTIL_DETAIL_UTF8_CHECKER_HPP
