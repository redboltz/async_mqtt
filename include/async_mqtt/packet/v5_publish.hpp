...
public:
    ...
    basic_publish_packet(
        ...
        pub::opts pubopts,
        properties props = {}
    ) : basic_publish_packet(0, force_move(topic_name), std::forward<BufferSequence>(payloads), pubopts, force_move(props)) {
    }
    ...
    std::vector<std::string> payload() const {
        ...
        for (auto const& p : payloads_) {
            ret.emplace_back(std::string{p});
        }
        return ret;
    }

    /**
     * @brief Get payload as buffer
     *        For advanced use.
     * @return payload
     */
