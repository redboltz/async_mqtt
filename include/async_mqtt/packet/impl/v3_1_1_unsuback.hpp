...
#if !defined(ASYNC_MQTT_PACKET_IMPL_V3_1_1_UNSUBACK_HPP)
#define ASYNC_MQTT_PACKET_IMPL_V3_1_1_UNSUBACK_HPP
...
template <std::size_t PacketIdBytes>
inline
constexpr std::size_t basic_unsuback_packet<PacketIdBytes>::num_of_const_buffer_sequence() {
    return 1; // all
}

template <std::size_t PacketIdBytes>
inline
typename basic_unsuback_packet<PacketIdBytes>::packet_id_t basic_unsuback_packet<PacketIdBytes>::packet_id() const {
    return endian_load<packet_id_t>(&all_[2]);
}

template <std::size_t PacketIdBytes>
inline
basic_unsuback_packet<PacketIdBytes>::basic_unsuback_packet(buffer buf) {
    // fixed_header
    if (buf.empty()) {
        throw make_error(
            errc::bad_message,
...
#endif // ASYNC_MQTT_PACKET_IMPL_V3_1_1_UNSUBACK_HPP
