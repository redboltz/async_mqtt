...
|===
|functionality\library|async_mqtt|async-mqtt5|mqtt_cpp|note

|High level easy to use APIs  support|x|x|-
|Low level detailed operation APIs  support|x|-|x
|MQTT Client APIs support|x|x|x
|MQTT Server(broker) APIs support|x|-|x|Send CONNACK, SUBACK, etc
|Auto adjust protocol behavior support|x|-|x|Receive Mamixum, Topic Alias, etc
...
|Offline publish support|x|-|x|Out of MQTT spec
