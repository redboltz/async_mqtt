...
#include <coro_broker/broker.hpp>
...
coro_broker(
    ...
    try {
        ...
        am::broker<
            epv_type
        > brk{vm["recycling_allocator"].as<bool>()};
        ...
        if (vm.count("tcp.port")) {
            ...
            as::co_spawn(
                accept_ioc.get_executor(),
                [&] () -> as::awaitable<void> {
                    while (true) {
                        auto epsp = std::make_shared<
                            am::basic_endpoint<
                                am::role::server,
                                2,
                                am::protocol::mqtt
                        ...
                        auto [ec] = co_await mqtt_ac->async_accept(
                            lowest_layer,
            ...
            as::co_spawn(
                ...
                [&] () -> as::awaitable<void> {
                    while (true) {
                        ...
                        epsp->set_read_buffer_size(vm["read_buf_size"].as<std::size_t>());
                        auto& lowest_layer = epsp->lowest_layer();
                        auto [ec] = co_await ws_ac->async_accept(
                            lowest_layer,
                            as::as_tuple(as::deferred)
                        );
                        if (ec) {
                            ...
                            bs::http::request<bs::http::string_body> request;
                            auto [ec, _] = co_await bs::http::async_read(
                                ws_layer.next_layer(),
                                sb,
                                request,
                                as::as_tuple(as::deferred)
                            ...
                            if (ec) {
                                ...
                                auto [ec] = co_await ws_layer.async_accept(
                                    request,
                                    as::as_tuple(as::deferred)
                                );
                                if (ec) {
            ...
            as::co_spawn(
                ...
                [&] () -> as::awaitable<void> {
                    while (true) {
                        ...
                        epsp->set_read_buffer_size(vm["read_buf_size"].as<std::size_t>());
                        auto& lowest_layer = epsp->lowest_layer();
                        auto [ec] = co_await mqtts_ac->async_accept(
                            lowest_layer,
                            as::as_tuple(as::deferred)
                        );
                        if (ec) {
                            ...
                            apply_socket_opts(lowest_layer);
                            auto [ec] = co_await epsp->next_layer().async_handshake(
            ...
            as::co_spawn(
                ...
                [&] () -> as::awaitable<void> {
                    while (true) {
                        ...
                        epsp->set_read_buffer_size(vm["read_buf_size"].as<std::size_t>());
                        auto& lowest_layer = epsp->lowest_layer();
                        auto [ec] = co_await wss_ac->async_accept(
                            lowest_layer,
                            as::as_tuple(as::deferred)
                        );
                        if (ec) {
                            ...
                            // TBD insert underlying timeout here
                            apply_socket_opts(lowest_layer);
                            auto [ec] = co_await epsp->next_layer().next_layer().async_handshake(
                                as::ssl::stream_base::server,
                                as::as_tuple(as::deferred)
                            );
                            if (ec) {
...
inline
void
run_broker(boost::program_options::variables_map const& vm) {
    as::io_context broker_ioc;
    as::any_io_executor exe{broker_ioc.get_executor()};
    as::co_spawn(
        exe,
        coro_broker(vm),
        as::detached
    ...
    std::thread th_broker {
        [&] {
            ...
            ASYNC_MQTT_LOG("mqtt_broker", trace) << "broker_ioc.run() finished";
        }
    };
    th_broker.join();
    ASYNC_MQTT_LOG("mqtt_broker", trace) << "th_broker joined";
}
