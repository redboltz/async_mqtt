...
struct session_state : std::enable_shared_from_this<session_state<Sp>> {
    ...
    publish(
        ...
        auto send_publish =
            ...
            (packet_id_type pid) mutable -> as::awaitable<void> {
                if (auto sp = wp.lock()) {
                    switch (version_) {
        ...
        if (offline_messages_empty_) {
            ...
            if (qos_value == qos::at_least_once ||
                ...
                auto [ec, pid] = co_await epsp.async_acquire_unique_packet_id(
                    as::as_tuple(as::use_awaitable)
                );
                if (!ec) {
                    co_await send_publish(pid);
                    co_return;
                }
                co_return;
            }
            else {
                co_await send_publish(0);
                co_return;
            }
        }

        // offline_messages_ is not empty or packet_id_exhausted
        std::unique_lock<mutex> g(mtx_offline_messages_);
    ...
    void clean() {
        ...
        {
            std::unique_lock<mutex> g(mtx_inflight_messages_);
    ...
    void subscribe(
        ...
        auto handle_ret =
            [&] {
                std::unique_lock<mutex> g{mtx_subs_map_};
    ...
    as::awaitable<void>
    send_will(bool no_delay = false) {
        if (!will_value_) co_return;
        if (no_delay) {
            co_await send_will_impl();
            co_return;
        }
        ...
        if (remain_after_close_ && wd_sec != 0) {
            ...
            auto [ec] = co_await tim_will_delay_.async_wait(as::as_tuple(as::deferred));
            if (!ec) {
                co_await send_will_impl();
            }
        }
...
private:
    ...
    send_will_impl() {
        ...
        if (will_sender_) {
            auto payloads = std::vector{force_move(payload)};
...
private:
    ...
    std::shared_ptr<as::steady_timer> tim_session_expiry_{std::make_shared<as::steady_timer>(exe_)};
