...
struct session_state : std::enable_shared_from_this<session_state<Sp>> {
    ...
    static std::shared_ptr<session_state<Sp>> create(
        ...
        sssp->update_will(timer_exe, will, will_expiry_interval);
        return sssp;
    }
    ...
    void become_offline(
        ...
        if (session_expiry_interval_ &&
            *session_expiry_interval_ != std::chrono::seconds(session_never_expire)) {
            ...
            tim_session_expiry_ = std::make_shared<as::steady_timer>(timer_exe_, *session_expiry_interval_);
    ...
    void renew(
        ...
        update_will(timer_exe_, force_move(will), will_expiry_interval);
    ...
    void update_will(
        ...
        if (will_value_ && will_expiry_interval) {
            tim_will_expiry_ = std::make_shared<as::steady_timer>(timer_exe, *will_expiry_interval);
