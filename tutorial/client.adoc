...
= {api-base}/++classasync__mqtt_1_1client.html#details++[Client]
...
CompletionToken parameters are `error_code` and `connack_packet` (optional). `connack_packet` has a value only if `error_code` is successful. When using CompletionToken such as `as::use_awaitable`, `as::use_future`, or `as::deferred`, the first `error_code` is converted to an exception. If you want the `error_code` as the return value, you can do the following:
...
CompletionToken parameters are error_code and {api-base}/++structasync__mqtt_1_1client_1_1pubres__t.html++[pubres_t].
...
In order to create QoS1 PUBLISH packet, you need to acquire packet identifier. The example code above uses {api-base}/++classasync__mqtt_1_1client.html#ae866b887888316e02f14487ec2892a23++[async_acquire_unique_packet_id()]. This is asynchnorous version. You can call it form anywhere you want. If all packet id is acquired, the CompletionToken parameter is nullopt. For convenience, sync version {api-base}/++classasync__mqtt_1_1client.html#ab160ebb19bd7e51f8460e59a88deff86++[acquire_unique_packet_id()] exists. Sync version must be called in the thread safe context. For example, if you pass the strand wrapped executor to the client on creation, in the async_function's callback handler is a thread safe context.

After async_publish() is completer, puback_opt of {api-base}/++structasync__mqtt_1_1client_1_1pubres__t.html++[pubres_t] is set. You can get puback packet.
...
In order to create QoS2 PUBLISH packet, you need to acquire packet identifier. In this example, {api-base}/++classasync__mqtt_1_1client.html#a107f857b734a01cfeb6f8fc7894e60f1++[async_acquire_unique_packet_id_wait_until()] is used. The CompletionToken parameter is packet identifier that not optional. If all packet identifiers are used, the function waits at least one of the packet identifier becomes usable again, and then invokes CompletionToken. It can help keeping user code simple.

After async_publish() is completer, pubrec_opt and pubcomp_opt of {api-base}/++structasync__mqtt_1_1client_1_1pubres__t.html++[pubres_t] are set. You can get pubrec and pubcomp packet.
...
After you called async_start() function, the received PUBLISH packets are stored in the clinet. You can get it using async_recv() function. If the no packets are stored, async_recv() waits until PUBLISH packet would be received.
...
If you want to close the socket forcibly, you can call async_close() function. For example, no packets are received from the broker unexpectedly, but the client side doesn't detect disconnection.
