...
BOOST_AUTO_TEST_CASE(v311_disconnect) {
...

    {
        ...
        am::error_code ec;
        auto p = am::v3_1_1::disconnect_packet{buf, ec};
        BOOST_TEST(!ec);
    ...
    auto p2 = am::v3_1_1::disconnect_packet();
    BOOST_CHECK(p == p2);
    BOOST_CHECK(!(p < p2));
    BOOST_CHECK(!(p2 < p));
...
BOOST_AUTO_TEST_CASE(v311_disconnect_error) {
    {
        am::buffer buf; // empty
        am::error_code ec;
        am::v3_1_1::disconnect_packet{buf, ec};
