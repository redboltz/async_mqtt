...
BOOST_AUTO_TEST_CASE(v311_pubcomp) {
...

    {
        ...
        am::error_code ec;
        auto p = am::v3_1_1::pubcomp_packet{buf, ec};
        BOOST_TEST(!ec);
        BOOST_TEST(p.packet_id() == 0x1234);
    ...
    BOOST_TEST(
        ...
        "v3_1_1::pubcomp{pid:4660}"
    );

    auto p2 = am::v3_1_1::pubcomp_packet{
        0x1234 // packet_id
    };
    auto p3 = am::v3_1_1::pubcomp_packet{
        0x1235 // packet_id
    };
    BOOST_CHECK(p == p2);
    BOOST_CHECK(!(p < p2));
...
BOOST_AUTO_TEST_CASE(v311_pubcomp_pid4) {
...

    {
        ...
        am::buffer buf{std::begin(expected), std::end(expected)};
        am::error_code ec;
        auto p = am::v3_1_1::basic_pubcomp_packet<4>{buf, ec};
        BOOST_TEST(!ec);
        BOOST_TEST(p.packet_id() == 0x12345678);
    ...
    BOOST_TEST(
        boost::lexical_cast<std::string>(p) ==
        "v3_1_1::pubcomp{pid:305419896}"
    );
}

BOOST_AUTO_TEST_CASE(v311_pubcomp_error) {
    {
        am::buffer buf; // empty
        am::error_code ec;
        am::v3_1_1::pubcomp_packet{buf, ec};
    ...
    {
        //                CP  RL  PID
        am::buffer buf{"\x70\x02\x12\x34\x56"sv}; // too long
        am::error_code ec;
        am::v3_1_1::pubcomp_packet{buf, ec};
        BOOST_TEST(ec == am::disconnect_reason_code::malformed_packet);
    }
