...
BOOST_AUTO_TEST_CASE(v5_pubcomp) {
...

    {
        ...
        am::error_code ec;
        auto p = am::v5::pubcomp_packet{buf, ec};
        BOOST_TEST(!ec);
        BOOST_TEST(p.packet_id() == 0x1234);
        BOOST_TEST(p.code() == am::pubcomp_reason_code::packet_identifier_not_found);
        BOOST_TEST(p.props() == props);
        ...
        BOOST_TEST(p.type() == am::control_packet_type::pubcomp);
    }
    BOOST_TEST(
        boost::lexical_cast<std::string>(p) ==
        "v5::pubcomp{pid:4660,rc:packet_identifier_not_found,ps:[{id:reason_string,val:some reason}]}"
    );

    auto p2 = am::v5::pubcomp_packet{
        0x1234, // packet_id
        am::pubcomp_reason_code::packet_identifier_not_found,
        props
    };
    auto p3 = am::v5::pubcomp_packet{
        0x1235, // packet_id
        am::pubcomp_reason_code::packet_identifier_not_found,
        props
    };
...
BOOST_AUTO_TEST_CASE(v5_pubcomp_pid4) {
    auto props = am::properties{
        am::property::reason_string("some reason")
    };
    auto p = am::v5::basic_pubcomp_packet<4>{
...

    {
        ...
        am::buffer buf{std::begin(expected), std::end(expected)};
        am::error_code ec;
        auto p = am::v5::basic_pubcomp_packet<4>{buf, ec};
        BOOST_TEST(!ec);
        BOOST_TEST(p.packet_id() == 0x12345678);
        BOOST_TEST(p.code() == am::pubcomp_reason_code::packet_identifier_not_found);
        BOOST_TEST(p.props() == props);
    ...
    BOOST_TEST(
        boost::lexical_cast<std::string>(p) ==
        "v5::pubcomp{pid:305419896,rc:packet_identifier_not_found,ps:[{id:reason_string,val:some reason}]}"
    );
}

BOOST_AUTO_TEST_CASE(v5_pubcomp_pid_only) {
    auto p = am::v5::pubcomp_packet{
        0x1234 // packet_id
    };
...
BOOST_AUTO_TEST_CASE(v5_pubcomp_pid_rc) {
    auto p = am::v5::pubcomp_packet{
        0x1234, // packet_id
    ...
    BOOST_TEST(
        boost::lexical_cast<std::string>(p) ==
        "v5::pubcomp{pid:4660,rc:success}"
    );
}

BOOST_AUTO_TEST_CASE(v5_pubcomp_prop_len_last) {
    char expected[] {
        0x70,                               // fixed_header
...
BOOST_AUTO_TEST_CASE(v5_pubcomp_error) {
    {
        am::buffer buf; // empty
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
    ...
    {
        //                CP  RL
        am::buffer buf{"\x70\x01"sv}; // remaining length buf mismatch
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
        BOOST_TEST(ec == am::disconnect_reason_code::malformed_packet);
    }
    {
        //                CP  RL
        am::buffer buf{"\x70\x03"sv}; // invalid remaining length
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
        BOOST_TEST(ec == am::disconnect_reason_code::malformed_packet);
    }
    {
        //                CP  RL  PID
        am::buffer buf{"\x70\x02\x12\x34"sv}; // valid
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
    ...
    {
        //                CP  RL  PID
        am::buffer buf{"\x70\x02\x12\x34\x00"sv}; // remaining length mismatch
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
        BOOST_TEST(ec == am::disconnect_reason_code::malformed_packet);
    }
    {
        //                CP  RL  PID     RC
        am::buffer buf{"\x70\x03\x12\x34\x00"sv}; // valid
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
        BOOST_TEST(ec == am::errc::success);
    }
    {
        //                CP  RL  PID     RC
        am::buffer buf{"\x70\x03\x12\x34\x84"sv}; // invalid rc
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
        BOOST_TEST(ec == am::disconnect_reason_code::malformed_packet);
    }
    {
        //                CP  RL  PID     RC
        am::buffer buf{"\x70\x03\x12\x34\x00\x00"sv}; // remaining length mismatch
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
        BOOST_TEST(ec == am::disconnect_reason_code::malformed_packet);
    }
    {
        //                CP  RL  PID     RC  PL
        am::buffer buf{"\x70\x04\x12\x34\x00\x00"sv}; // valid
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
        BOOST_TEST(ec == am::errc::success);
    }
    {
        //                CP  RL  PID     RC  PL
        am::buffer buf{"\x70\x05\x12\x34\x00\x00\x00"sv}; // property length mismatch
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
        BOOST_TEST(ec == am::disconnect_reason_code::malformed_packet);
    }
    {
        //                CP  RL  PID     RC  PL
        am::buffer buf{"\x70\x05\x12\x34\x00\x02\x00"sv}; // property length mismatch
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
        BOOST_TEST(ec == am::disconnect_reason_code::malformed_packet);
    }
    {
        //                CP  RL  PID     RC  PL
        am::buffer buf{"\x70\x07\x12\x34\x00\xff\xff\xff\x80"sv}; // over property length
        am::error_code ec;
        am::v5::pubcomp_packet{buf, ec};
        BOOST_TEST(ec == am::disconnect_reason_code::malformed_packet);
    }
