...
using sub_con_online_map = am::multiple_subscription_map<sub_con_online_cref, int, sub_con_online_cref_hasher, sub_con_online_cref_equal_to >;
using sub_con_offline_map = am::multiple_subscription_map<sub_con_offline_cref, int, sub_con_offline_cref_hasher, sub_con_offline_cref_equal_to >;
...
using mi_sub_con_online = mi::multi_index_container<
    ...
    mi::indexed_by<
        mi::ordered_non_unique<
            ...
            BOOST_MULTI_INDEX_MEMBER(sub_con_online, am::buffer, topic_filter)
        >,
        mi::ordered_non_unique<
            mi::tag<tag_con>,
...
using mi_sub_con_offline = mi::multi_index_container<
    ...
    mi::indexed_by<
        ...
        mi::ordered_non_unique<
            mi::tag<tag_cid>,
            BOOST_MULTI_INDEX_MEMBER(sub_con_offline, am::buffer, client_id)
        >,
        // Allow multiple topic_filters for the same client id
        mi::ordered_non_unique<
            mi::tag<tag_topic_filter>,
            BOOST_MULTI_INDEX_MEMBER(sub_con_offline, am::buffer, topic_filter)
        >,
        ...
        [&](
            ...
            if ((it == idx.end()) || it->con != con || it->topic_filter != topic_filter) {
                ...
                return std::make_pair(
                    idx.emplace_hint(it, con, am::force_move(topic_filter), subopts, sid),
                    true
                );
            }
            else {
    ...
    // unsubscribe (con1 "a/b/c")
    {
        ...
        m.find(
            ...
            [&](sub_con_online_cref sc, int /*value*/) {
                ...
                BOOST_TEST(sc.get().subopts.get_qos() == am::qos::at_most_once);
    ...
    // update subscribe (con2 "a/b/c")
    {
        ...
        m.find(
            ...
            [&](sub_con_online_cref sc, int /*value*/) {
                BOOST_TEST(entries.erase(*sc.get().con) == 1);
                BOOST_TEST(sc.get().con == con2);
                BOOST_TEST(sc.get().subopts.get_qos() == am::qos::at_least_once);
